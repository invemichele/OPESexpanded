...
beta = 1
s_space,fes=np.loadtxt('data/model-fes.data',usecols=(0,1),unpack=True)
...
centers=np.array([(s_max-s_min)/(num_umb-1)*i+s_min for i in range(num_umb)])
def Gauss(s):
...
deltaF=-np.log(deltaF/max(deltaF))
print('#centers deltaF #N=%d'%len(centers))
for j in range(len(centers)):
  print(centers[j],deltaF[j])
