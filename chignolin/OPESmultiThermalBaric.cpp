...
private:
//========================================
  bool cut_corner_; //TODO make this general, so that can be set from plumed.dat
  const double from_bar_=0.06022140857;
  const double low_temp_=500;
  const double high_temp_=800;
  const double low_pres_=1*from_bar_;
  const double high_pres_=1000*from_bar_;
  const double coeff_=(high_pres_-low_pres_)/(high_temp_-low_temp_);
//========================================
...
//miscellaneous
  ...
  keys.addFlag("CUT_CORNER",false,"avoid region of high temperature and low pressure");
...
//work flag
  parseFlag("CUT_CORNER",cut_corner_);
  if(cut_corner_);
    plumed_massert(border_weight_==1,"CUT_CORNER not compatible with BORDER_WEIGHT");
...
//Bibliography
  ...
  if(cut_corner_)
    log.printf(" -- CUT_CORNER: dropping form target any (temp,pres) below the line obtained from ( %g K, %g bar ) and ( %g K, %g bar )\n",low_temp_,low_pres_/from_bar_,high_temp_,high_pres_/from_bar_);
}
...
void OPESmultiThermalBaric::calculate()
{
  ...
  {
    const double line_i=(1./(kb*beta_p_[i])-low_temp_)*coeff_+low_pres_;
    for(unsigned j=0; j<steps_pres_; j++)
    {
      if(cut_corner_ && pres_p_[j]<line_i)
        continue;
    ...
    for(unsigned i=0; i<steps_beta_; i++)
    {
      ...
      for(unsigned j=0; j<steps_pres_; j++)
      {
        if(pres_p_[j]>=line_i)
          ...
          log.printf(" --- Discarding from target deltaF_%d_%d at ( %7.3f K, %7.3f bar )\n",i+1,j+1,1./(kb*beta_p_[i]),pres_p_[j]/from_bar_);
      }
    }
  }
  else
...
//print some info
  ...
  if(cut_corner_)
    log.printf("  Total deltaFs = %g instead of %d (tot discarded %g )\n",tot_steps_,steps_beta_*steps_pres_,steps_beta_*steps_pres_-tot_steps_);
  else
