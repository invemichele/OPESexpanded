...
#part='-R "select[model==EPYC_7742]"'
#part='-R "span[ptile=128]"'
...
# Commands
nsteps="-nsteps 12500000" #about 20h on daint (no GPU)
#nsteps=""
exe="`which gmx_mpi` mdrun"
Filename=chignolin
Inputname=input-chignolin
...
res=""
res="yes"
if [ ! -z "$res" ] && [ `ls ${Project}*.cpt |wc -l` -gt 0 ]
then
...
then
  ...
  sb=_sbatchme.sh
  echo -e "#!/bin/bash\nsrun $hypt ${mpi_cmd}\n${extra_cmd}" > $sb
  submit="sbatch -C mc -o $outfile -J $jname -n $ncore -t $max_t:00 $singleton $sb"
