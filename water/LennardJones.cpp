...
class LennardJones : public Colvar {
  bool pbc;
  bool serial;
  std::unique_ptr<NeighborList> nl;
  bool invalidateList;
  bool firsttime;
  double rcut, rcut2, sigma, epsilon;
public:
  explicit LennardJones(const ActionOptions&);
  ~LennardJones();
// active methods:
...
PLUMED_REGISTER_ACTION(LennardJones,"LENNARDJONES")

void LennardJones::registerKeywords( Keywords& keys ) {
  ...
  keys.add("compulsory","RCUT","Cutoff");
  keys.add("compulsory","EPSILON","Epsilon");
  keys.add("compulsory","SIGMA","Sigma");
  keys.add("atoms","GROUPA","First list of atoms");
  keys.add("atoms","GROUPB","Second list of atoms (if empty, N*(N-1)/2 pairs in GROUPA are counted)");
}

LennardJones::LennardJones(const ActionOptions&ao):
...
// deriv.resize(getPositions().size());
  ...
  #pragma omp parallel num_threads(nt)
  {
    ...
    #pragma omp for reduction(+:energy) nowait
	...
	dfunc=-48*epsilon*r6i*(r6i-0.5)/r2;
        energy += 4*epsilon*r6i*(r6i-1);
